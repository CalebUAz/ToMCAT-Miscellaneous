...
plt.savefig(os.path.join(output_path, 'hexbin_plot.png'), bbox_inches='tight')
plt.show()
...
plt.savefig(os.path.join(output_path, 'histogram_arousal.png'), bbox_inches='tight')
plt.show()
...
std_arousal = master_df.groupby('image_path')['arousal_score'].std()
plt.figure(figsize=(12, 8))
mean_arousal.plot(kind='bar', color='skyblue', edgecolor='black', yerr=std_arousal, capsize=5)
plt.title('Bar Graph of Mean Arousal Score per Image with Standard Deviation')
plt.xlabel('Image Path')
plt.ylabel('Mean Arousal Score')
plt.xticks(rotation=45)
plt.savefig(os.path.join(output_path, 'bar_graph_arousal.png'), bbox_inches='tight')
plt.show()

# Bar graph for mean valence_score of each image
mean_valence = master_df.groupby('image_path')['valence_score'].mean()
std_valence = master_df.groupby('image_path')['valence_score'].std()
plt.figure(figsize=(12, 8))
mean_valence.plot(kind='bar', color='skyblue', edgecolor='black', yerr=std_valence, capsize=5)
plt.title('Bar Graph of Mean Valence Score per Image with Standard Deviation')
plt.xlabel('Image Path')
plt.ylabel('Mean Valence Score')
plt.xticks(rotation=45)
plt.savefig(os.path.join(output_path, 'bar_graph_valence.png'), bbox_inches='tight')
plt.show()